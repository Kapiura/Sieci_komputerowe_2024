...
int main() {
  struct sockaddr_in serwer = {.sin_family = AF_INET,
  ...
  while (1) {
    ...
    if (recvfrom(socket_, buffer, sizeof(buffer), 0, (struct sockaddr *)&client,
                 &len) < 0) {
    ...
    if (sendto(socket_, sendbuffer, 7000, 0, (struct sockaddr *)&client, len) <
        0) {
      perror("sendto() ERROR");
      exit(5);
    }
